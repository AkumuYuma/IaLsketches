...
genAlg::Popolazione::Popolazione(std::string target, int maxPop, float mutationRate) : target(target), maxPop(maxPop), mutationRate(mutationRate)  {
  // Inizializzo un array di individui 
  for (int i = 0; i < maxPop; i++) {
    individui.push_back(Individuo(target.size(), target)); 
  }

  std::sort(individui.begin(), individui.end()); 
  migliore = individui[0].geni;
}

void genAlg::Popolazione::nuovaGenerazione() {
  std::vector<genAlg::Individuo> nuovaGen; 
